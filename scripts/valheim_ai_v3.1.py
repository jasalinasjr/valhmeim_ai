...
class ValheimSimpleEnv(gym.Env):
    ...
    def _compute_reward(self, current_detections: Counter, current_health: float,
        ...
        # Resource collection
        resource_reward = sum(
            (REWARD_WEIGHTS.get("wood_bonus", 2.0) if item in ["wood", "stone"] else 1.5) *
            max(0, current_detections.get(item, 0) - self.last_detections.get(item, 0))
            for item in ITEMS
        )
        ...
        # Kill rewards
        kill_reward = sum(
            REWARD_WEIGHTS.get("kill_bonus", 8.0) *
            max(0, self.last_detections.get(enemy, 0) - current_detections.get(enemy, 0))
        ...
        if health_delta > 0.05:
            reward += REWARD_WEIGHTS.get("health_gain_bonus", 4.0)
        elif health_delta < -0.05:
            reward += REWARD_WEIGHTS.get("health_loss_penalty", -3.0)

        # Enemy visible penalty
        enemy_visible = sum(1 for e in ENEMIES if e in current_detections)
        reward += enemy_visible * REWARD_WEIGHTS.get("enemy_visible_penalty", -1.5)
        ...
        if self.last_preprocessed is not None:
            ...
            curiosity_reward = REWARD_WEIGHTS.get("curiosity_scale", 0.4) * (np.mean(diff) / 255.0)
    ...
    def step(self, action):
        ...
        if self.current_step % 100 == 0:
            resource_r = sum(
                (REWARD_WEIGHTS.get("wood_bonus", 2.0) if i in ["wood", "stone"] else 1.5) *
            ...
            health_delta = current_health - self.last_health_proxy
            health_r = (REWARD_WEIGHTS.get("health_gain_bonus", 4.0) if health_delta > 0.05 else
                       REWARD_WEIGHTS.get("health_loss_penalty", -3.0) if health_delta < -0.05 else 0.0)
            curiosity_r = 0.0
            if self.last_preprocessed is not None:
                diff = np.abs(current_preprocessed.astype(np.float32) - self.last_preprocessed.astype(np.float32))
                curiosity_r = REWARD_WEIGHTS.get("curiosity_scale", 0.4) * (np.mean(diff) / 255.0)
            ...
            logger.info(f"Reward Breakdown @ {self.current_step}: Time={REWARD_WEIGHTS.get('time_penalty',-0.01):.2f} | "
                        ...
                        f"EnemyVis={-1.5*sum(1 for e in ENEMIES if e in current_detections):.2f} | "
        ...
        terminated = False
        truncated = self.current_step > 5000   # Increased slightly
