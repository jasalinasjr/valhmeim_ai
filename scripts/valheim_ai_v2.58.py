...
class ValheimSimpleEnv(gym.Env):
    ...
    def _update_capture_region(self):
        """Update capture region using window detection with fallback"""
        region = find_valheim_window()
        if region:
            self.capture_region = region
        elif not getattr(self, 'capture_region', None):
            self.capture_region = CAPTURE_REGION.copy()
    ...
    def _compute_reward(self, current_detections: Counter, current_health: float, action: int) -> float:
        """Fixed reward function with all variables properly handled"""
    ...
    def step(self, action):
        ...
        # Compute reward with action passed
        reward = self._compute_reward(current_detections, current_health, action)
